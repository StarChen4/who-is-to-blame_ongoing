...
| 英雄名称 | 玩家表格 | 从英雄图片的 `data-for` 属性获取，或从图片URL提取 `/heroes/{name}.png` | "Windranger", "Puck" |
...
**英雄名称获取方法（重要）**:
- ❌ **不要使用** `alt` 属性，它总是空的
- ✅ **推荐方法1**: 从 `<img>` 的 `data-for` 属性获取，如 `data-for="Windranger"`
- ✅ **推荐方法2**: 从图片URL提取，如 `https://cdn.../heroes/windrunner.png` → `windrunner`
- 注意：URL中的名称是内部名（如 `windrunner`），`data-for` 是显示名（如 `Windranger`）
