...
| 英雄名称 | 玩家表格 | ⚠️ 见下方说明 | "Tidehunter", "Dazzle" |
...
**⚠️ 英雄名称获取警告**:
- OpenDota页面的英雄信息通过图标显示，在a11y快照中**可能无法直接获取英雄名称文本**
- 玩家-英雄映射关系**必须从STRATZ获取**（见下方"玩家-英雄映射数据"章节）
- 不要依赖OpenDota概览页获取英雄名称，否则可能导致映射错误
...
### 为什么使用STRATZ而非OpenDota

| 数据源 | 英雄名称获取方式 | 可靠性 |
|-------|----------------|-------|
| OpenDota | 英雄图标显示，a11y快照中可能无法获取文本 | ❌ 不可靠 |
| STRATZ | 英雄名称直接作为image元素描述显示 | ✅ 可靠 |

**结论**: 玩家-英雄映射关系**必须从STRATZ获取**，不要依赖OpenDota。
...
| 数据项 | 页面位置 | 获取方式 | 示例 |
|-------|---------|---------|------|
| 玩家昵称 | 对抗区域玩家链接 | `link "XinG"` | "XinG", "ssdsdbcx" |
| 英雄名称 | 对抗区域英雄图片 | `image "斯温"` | "斯温", "痛苦女王" |
| 阵营 | 对抗区域左右分组 | 左侧为天辉，右侧为夜魇 | - |

### 注意事项

- **必须先获取STRATZ的玩家-英雄映射**，再进行后续数据分析
- STRATZ的英雄名称为中文，与OpenDota数据对接时注意英雄名称一致性
- 对抗区域同时显示KDA和经济数据，可辅助验证映射正确性
...
| 异常情况 | 处理方式 |
|---------|---------|
| **STRATZ无法访问** | **❌ 无法继续分析**，必须等待STRATZ恢复 |
| STRATZ玩家-英雄映射不完整 | 尝试截图识别，或提示用户手动确认 |
| 比赛未分析 | 提示用户等待或手动触发分析 |
...
1. **⭐STRATZ玩家-英雄映射** (必需优先) - 获取玩家和英雄的对应关系
2. **STRATZ选人阶段** (必需) - 获取选人阶段信息，用于选人责任判定
3. **OpenDota概览页** (必需) - 判断胜负、获取基础数据（但不依赖它获取英雄名称）
4. **OpenDota分路页** (必需) - 确定位置、获取对线数据
5. **OpenDota表现页** (重要) - 获取辅助数据
6. **OpenDota团战页** (重要) - 获取团战数据
7. **OpenDota视野页** (重要) - 获取4/5号位的视野数据
8. **OpenDota曲线图页** (可选) - 获取经济曲线
9. **Dota2.Tools** (并发) - 获取5个英雄的克制数据(可并发请求)

**⚠️ 重要**: STRATZ必须在OpenDota之前获取，以确保玩家-英雄映射关系正确。

总耗时估计: 25-45秒(取决于网络和是否需要触发分析)
